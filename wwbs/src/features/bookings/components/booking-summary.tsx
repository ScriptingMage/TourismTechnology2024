...
export const BookingSummary = ({ booking, bookingStages }: BookingSummaryProps) => {
  return (
    <div className="border border-gray-500 p-4 w-full">
      ...
      <div>
        <span className="font-bold">Start Date:</span> {booking.startDate.toLocaleDateString()}
      </div>
      <div>
        ...
        {bookingStages.map((bookingStage, index) => {
          ...
          return (
            <div key={bookingStage.id}>
              <h4 className="text-xl font-bold mb-2">Stage {index+1}: {bookingStage.startDate.toLocaleDateString()} - {bookingStage.endDate.toLocaleDateString()}</h4>
